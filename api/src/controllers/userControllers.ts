...
export async function loginUser(req: Request, res: Response, next: NextFunction) {
	try {
		...
		const user = await User.findOne({ where: { email: req.body.email }/*, include: [{ model: movieInstance, as: 'MOVIES' }]*/ }) as unknown as { [key: string]: string };
		const { id } = user;
		const email = user.email;
		const fullname = user.fullname;
		const username = user.username;
		const userInfo = { id, email, fullname, username };
...
// 		const entry: any = await User.findOne({ where: { id }, include: [{ model: movieInstance, as: 'Movies' }] });
...
// 	try {
// 		const user = await User.findOne({ where: { id: req.cookies.id }, include: [{ model: movieInstance, as: 'MOVIES' }] }) as unknown as { [key: string]: string };
